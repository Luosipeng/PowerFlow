...
mutable struct Transformer3W <: AbstractComponent
    ...
    function Transformer3W(index, name, std_type, hv_bus, mv_bus, lv_bus, 
        ...
        # 设置默认值
        self.parallel = 1
        self.max_loading_percent = 100.0
        self.df = 1.0
        self.in_service = true
        self.oltc = false
        self.power_station_unit = false
...
# 分接头配置结构体
mutable struct TapSettings
    available::Bool            # 是否配置分接头
    side::String               # "hv"/"mv"/"lv"
    neutral_pos::Int           # 中间档位
    min_pos::Int               # 最小档位
    max_pos::Int               # 最大档位
    step_percent::Float64      # 每档电压变化率
    step_degree::Float64       # 每档相角变化
    current_pos::Int           # 当前档位
    at_star_point::Bool        # 中性点是否接地
    phase_shifter::Bool        # 是否为移相器
end
# 开关结构
