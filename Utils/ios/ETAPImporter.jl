    ...
    try
        ...
        for (i, row) in enumerate(eachrow(df))
            try
                ...
                max_vm_pu = haskey(row, :max_vm_pu) ? safe_get_value(row[:max_vm_pu], 1.1, Float64) : 1.1
                min_vm_pu = haskey(row, :min_vm_pu) ? safe_get_value(row[:min_vm_pu], 0.9, Float64) : 0.9
