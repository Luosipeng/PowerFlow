...
function pfsoln(baseMVA::Float64, bus0::Matrix{Float64}, gen0::Matrix{Float64}, branch0::Matrix{Float64},load0::Matrix{Float64}, Ybus, Yf, Yt, V, ref, pv, pq)
    # Define named indices into bus, gen, branch matrices
    PQ, PV, REF, NONE, BUS_I, BUS_TYPE, PD, QD, GS, BS, BUS_AREA, VM, VA, BASE_KV, ZONE, VMAX, VMIN, LAM_P, LAM_Q, MU_VMAX, MU_VMIN = PowerFlow.idx_bus()
    ...
    Pd_gbus, Qd_gbus = PowerFlow.total_load(bus[gbus, :],Ld[gbus,:]);
    ...
    if length(on) > 1
     ...
     for k in eachindex(ref)
         ...
         loadm = loadm'
         Pd_refk, Qd_refk = PowerFlow.total_load(busm,loadm)
